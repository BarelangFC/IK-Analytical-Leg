...
def publish():
    ...
    data.name = ['r_hip_yaw', 'r_hip_roll', 'r_ank_pitch_back', 'r_knee', 'r_ank_roll']
    data.position = [angle[0], angle[1], angle[2], angle[3], angle[1]]
    pub.publish(data)
...
def FK(angle):
    ...
    knee_from_hip = TF(rot_axis='y', q=-angle[3], dz=-0.275)
    knee_up = hip.dot(knee_from_hip) #R34
    kneeDown_from_up = TF(rot_axis='y', q=angle[4], dz=-0.16)
    ...
    ankle_roll_from_ankle_pitch = TF(rot_axis='x', q=-angle[6])
...
def main(t_values):
    ...
    for t in t_values:
        ...
        markerbasics_object = makeBox(testCnt,xyz,status)
...
Succeed = main(t_points)
print ("total berhasil",Succeed)
#plt.show()    
