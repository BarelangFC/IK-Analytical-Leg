...
def publish():
    ...
    data.position = [angle[0], angle[1], angle[5], angle[2],angle[6]]
    #data.position = [0, 0, 1, 0, 0]
    pub.publish(data)
...
def main(t_values, target, yawt):
    ...
    UPPER_HIP = 0.107       # Jarak hip yaw ke hip roll pitch
    HIP_TO_KNEE = 0.311      # Panjang link upper leg
    KNEE_TO_ANKLE = 0.311    # Panjang link lower leg
    ANKLE_TO_SOLE = 0.09   # Jarak ankle ke sole
    UP_KNEE_TO_DOWN_KNEE = 0.0 #jarak joint pada lutut
    ...
    for t in t_values:
        ...
        zgear = (0.16*np.sin(qrol2))
        #z_from_hip = z_from_hip2 + zgear
        #print (zgear)
        '''
        ...
        # Forward kinematic
        base = TF()
        hip_yaw_from_base = TF(rot_axis='z', q=qHipYaw, dy=0.105)
        hip_yaw = base.dot(hip_yaw_from_base) #R01
        hip_roll_from_hip_yaw = TF(rot_axis='x', q=qHipRoll, dz=-0.0625)
        ...
        knee_from_hip = TF(rot_axis='y', q=-qKneeUp, dz=-0.275)
        knee_up = hip.dot(knee_from_hip) #R34
        kneeDown_from_up = TF(rot_axis='y', q=-qKneeDown, dz=-0.16)
        knee_Down = knee_up.dot(kneeDown_from_up) 
        ankle_pitch_from_knee = TF(rot_axis='y', q=qAnklePitch, dz=-0.275)
...
#points_set_1 = a([[0, 0.105, -0.8], [0, 0.105, -0.69], [0, 0.405, -0.580], [0, 0.405, -0.580]])
points_set_1 = a([[0, 0.105, -0.8], [0, 0.105, -0.69], [0.1, 0.105, -0.62], [0.1, 0.105, -0.62]])
