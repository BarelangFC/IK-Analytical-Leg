...
def publish():
    ...
    data.name = ['l_hip_yaw', 'l_hip_roll', 'l_ank_pitch_back', 'l_knee', 'l_ank_roll']
    data.position = [angle[0], angle[1], angle[2], angle[5], angle[6]]
    pub.publish(data)
    '''
    pub1 = rospy.Publisher('/humanoid/l_hip_yaw_position/command', Float64, queue_size=100)
    pub2 = rospy.Publisher('/humanoid/l_hip_roll_position/command', Float64, queue_size=100)
    pub4 = rospy.Publisher('/humanoid/l_ank_pitch_back_position/command', Float64, queue_size=100)
    pub3 = rospy.Publisher('/humanoid/l_knee_position/command', Float64, queue_size=100)
    pub5 = rospy.Publisher('/humanoid/l_ank_roll_position/command', Float64, queue_size=100)
...
def FK(angle):
    ...
    hip_roll_from_hip_yaw = TF(rot_axis='x', q=angle[1], dz=-0.0615)
...
def main(t_values):
    ...
    UPPER_HIP = 0.0615       # Jarak hip yaw ke hip roll pitch
    HIP_TO_KNEE = 0.275      # Panjang link upper leg
    KNEE_TO_ANKLE = 0.275    # Panjang link lower leg
    ANKLE_TO_SOLE = 0.0465   # Jarak ankle ke sole
    ...
    for t in t_values:
        sleep(1)
        ub = [0.5,0.2,1.5,0,0,1,0]
        lb = [-0.5,-0.1,-1.5,0,0,-1,0]
        randomangle = np.random.uniform(low = lb, high = ub, size = 7)
        #print (randomangle)
        ...
        markerbasics_object = makeBox(a,xyz,status)
        marker_objectlisher.publish(markerbasics_object)
        
        #kaki = [kaki, kaki2, kaki3]
